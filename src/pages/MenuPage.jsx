...
const AnnouncementModal = ({ t, announcements, onClose }) => {
            ...
            <div className="bg-white w-full max-w-lg rounded-xl shadow-2xl p-6 text-center animate-slide-up relative" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
                ...
                <div className="relative h-80 overflow-hidden">
                    {announcements.map((ann, index) => (
                        <div key={ann.id || index} className="absolute top-0 left-0 w-full h-full transition-transform duration-500 ease-in-out" style={{ transform: `translateX(${(index - currentIndex) * 100}%)` }}>
...
const CartModal = ({ cart, t, lang, menuData, subTotal, transactionFee, totalAmount, settings, onClose, onUpdateQuantity, onRemove, onSubmitOrder }) => {
    ...
    const handleGetRecommendation = async () => {
        ...
        const recommendationRequest = { language: translations[lang]?.language || "English", cartItems: cart.length > 0 ? cartItemNames : null, availableItems: availableMenuItems };
        ...
        try {
            ...
            if (error.name === 'AbortError') {
                setRecommendation("AI小助手忙線中，請稍後再試。");
            } else {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-end" onClick={onClose}>
            <div className="bg-gray-50 w-full max-w-md h-full flex flex-col shadow-2xl animate-slide-in-right" onClick={e => e.stopPropagation()}>
                <header className="p-4 border-b flex justify-between items-center">
                    ...
                    <button onClick={onClose} className="flex items-center gap-2 text-orange-600 font-semibold hover:text-orange-700"><ArrowLeft size={18} />{t.continueOrdering}</button>
                </header>
                <main className="flex-1 overflow-y-auto p-4 space-y-4">
                    {cart.length === 0 ? (
                        <div className="text-center text-gray-500 py-10"><ShoppingCart size={48} className="mx-auto text-gray-300" /><p className="mt-4">{t.emptyCart}</p></div>
                    ) : (
                        cart.map(item => (
                    ...
                    {settings.isAiEnabled && (
                        <div className="pt-4">
                            <button onClick={handleGetRecommendation} disabled={isRecommending} className="w-full bg-blue-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-600 transition-colors duration-300 flex items-center justify-center disabled:bg-blue-300 disabled:cursor-wait">
                                ...
                                {cart.length > 0 ? t.getRecommendation : '不知道吃什麼？讓AI推薦！'}
                            </button>
                        </div>
                    )}
                ...
                <footer className="p-4 bg-white border-t">
                    <div className="flex justify-between items-center mb-1"><span className="text-md text-gray-600">小計</span><span className="text-md font-medium text-gray-800">${subTotal}</span></div>
                    <div className="flex justify-between items-center mb-4"><span className="text-md text-gray-600">服務費 ({settings.transactionFeePercent || 0}%)</span><span className="text-md font-medium text-gray-800">${transactionFee}</span></div>
                    <div className="flex justify-between items-center mb-4 border-t pt-4"><span className="text-lg font-semibold text-gray-800">{t.total}</span><span className="text-2xl font-bold text-orange-500">${totalAmount}</span></div>
                    <button onClick={onSubmitOrder} disabled={cart.length === 0} className="w-full bg-green-500 text-white text-lg font-bold py-3 rounded-lg hover:bg-green-600 transition-colors duration-300 disabled:bg-gray-300 disabled:cursor-not-allowed">{t.submitOrder}</button>
                </footer>
            </div>
        </div>
    );
