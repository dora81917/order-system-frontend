...
const MenuItemForm = ({ item, onSave, onCancel }) => {
    ...
    useEffect(() => {
        ...
        if (item && item.options) {
            const optionsArray = Array.isArray(item.options) ? item.options : String(item.options).split(',');
...
const StorefrontSettings = () => {
    ...
    const handleSaveAll = async () => {
        ...
        // 2a. 刪除已移除的公告
        const announcementsToDelete = originalAnnouncements.filter(origAnn => !currentIds.has(origAnn.id));
